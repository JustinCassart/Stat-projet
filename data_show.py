...
def data_show(data):
    ...
    for i in range(len(data)):
        ...
        # sub_fig.suptitle("Processor %d plot data"%(i+1))
...
def errorbar_show(data):
    ...
    for i in range(len(data)):
        ...
        x = data[i][:,0]
        # x = data[:,0]
        y_mean = utils.means(data, i)
        y_err = utils.errors(data, i)
...
def show_hist(processor, n_triangles):
    n_triangles = int(n_triangles/10000) - 1
    y = processor[n_triangles, 1:]
    begin = int(min(y))
    end = int(max(y))
    step = end // 30
    admissible = [i for i in range(begin, end, step)]
    a = [get_index(i, admissible) for i in y]
    plt.hist(a, align="left", bins=len(admissible) - 1)
    plt.xticks(range(len(admissible)), admissible)
    plt.show()

def show_gauss(_mu, _rho):
    for i in range(len(_mu)):
        x = numpy.linspace(_mu[i] - 3*_rho[i], _mu[i] + 3*_rho[i])
        plt.plot(x, stats.norm.pdf(x, _mu[i], _rho[i]), label = "GPU {}".format(i+1), color=COLOR[i][0])
